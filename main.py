...
if df is not None:
    ...
    def render_stats_tab(f_data, key_suffix):
        ...
        with col_left:
            st.write("### 📊 球種別分析")
            st.markdown("<style>.stTable td, .stTable th { text-align: center !important; } div[data-testid='stTable'] { display: flex; justify-content: center; }</style>", unsafe_allow_html=True)
        ...
        if mode == "2ストライク時のみ":
            # 追い込まれてからのデータのみ抽出
            two_strike_data = f_data[f_data['Strikes'] == 2].copy()
            if two_strike_data.empty:
                st.info("2ストライクのデータがありません。")
            else:
                cnt_map = pd.crosstab(two_strike_data['Count'], two_strike_data['TaggedPitchType'])
                total_series = two_strike_data['TaggedPitchType'].value_counts()
                total_df = pd.DataFrame(total_series).T
                total_df.index = ["2ストライク全体"]
                order = ["0-2", "1-2", "2-2", "3-2", "2ストライク全体"]
                cnt_map_with_total = pd.concat([cnt_map, total_df]).reindex(index=order, fill_value=0)
                plot_data = cnt_map_with_total.div(cnt_map_with_total.sum(axis=1).replace(0, 1), axis=0) * 100
                st.bar_chart(plot_data)
        else:
            # 通常モード（全カウント）
            cnt_map = pd.crosstab(f_data['Count'], f_data['TaggedPitchType'])
            total_series = f_data['TaggedPitchType'].value_counts()
            total_df = pd.DataFrame(total_series).T
            total_df.index = ["全体"]
            order = ["0-0", "1-0", "2-0", "3-0", "0-1", "1-1", "2-1", "3-1", "0-2", "1-2", "2-2", "3-2", "全体"]
            cnt_map_with_total = pd.concat([cnt_map, total_df]).reindex(index=order, fill_value=0)
            plot_data = cnt_map_with_total.div(cnt_map_with_total.sum(axis=1).replace(0, 1), axis=0) * 100
            st.bar_chart(plot_data)
    ...
    # 各タブの実行
    with tabs[0]: render_stats_tab(render_filters(df[df['DataCategory']=="SBP"], "sbp"), "sbp")
    with tabs[1]: render_stats_tab(render_filters(df[df['DataCategory']=="vs"], "vs"), "vs")
    with tabs[2]: render_visual_tab(render_filters(df[df['DataCategory']=="PBP"], "pbp", show_side=False, show_runner=False))
    with tabs[3]: render_visual_tab(render_filters(df[df['DataCategory']=="ptc"], "ptc", show_side=False, show_runner=False))
else:
