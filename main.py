...
def render_risk_management_grid(f_data):
    ...
    c1, c2 = st.columns(2)
    with c1:
        fig = px.bar(pd.DataFrame(side_list), y='対象', x='割合(%)', color='カテゴリ', orientation='h', color_discrete_map=color_map, category_orders={'カテゴリ': cat_order}, height=220)
        fig.update_layout(showlegend=False, margin=dict(l=0, r=0, t=10, b=10))
        st.plotly_chart(fig, use_container_width=True)
    with c2:
        fig = px.bar(pd.DataFrame(pitch_list), y='球種', x='割合(%)', color='カテゴリ', orientation='h', color_discrete_map=color_map, category_orders={'カテゴリ': cat_order}, height=220)
        fig.update_layout(showlegend=True, margin=dict(l=0, r=0, t=10, b=10), legend_title="")
        st.plotly_chart(fig, use_container_width=True)
...
def render_stats_tab(f_data, key_suffix):
    ...
    # 表示用フォーマット
    disp = summary.copy()
    for c in ['投球割合', 'ストライク率', 'スイング率', 'Whiff %']: disp[c] = (summary[c] * (100 if c!='投球割合' else 1)).apply(lambda x: f"{x:.1f} %")
    
    col_l, col_r = st.columns([2, 1])
    with col_l:
        st.write("### 📊 球種別分析")
        st.table(disp[['投球数', '投球割合', '平均球速', '最速', 'ストライク率', 'Whiff %']])
        render_risk_management_grid(f_data) # 表の下にリスク管理
    
    with col_r:
        st.write("### 🥧 投球割合")
        plt.clf(); fig, ax = plt.subplots(figsize=(3.5, 3.5)) # 小さく調整
...
if df is not None:
    tabs = st.tabs(["🔹 SBP", "🔹 実戦/PBP"])
    with tabs[0]:
        data_sbp = df[df['DataCategory']=="SBP"]
        p_list = sorted([str(p) for p in data_sbp['Pitcher'].unique() if p != "Unknown"])
        c1, c2 = st.columns(2)
        p = c1.selectbox("投手", ["すべて"] + p_list, key="p_sbp")
        d = c2.selectbox("日付", ["すべて"] + sorted(data_sbp['Date'].unique().astype(str), reverse=True), key="d_sbp")
        if p != "すべて": data_sbp = data_sbp[data_sbp['Pitcher'] == p]
        if d != "すべて": data_sbp = data_sbp[data_sbp['Date'].astype(str) == d]
        render_stats_tab(data_sbp, "sbp")
