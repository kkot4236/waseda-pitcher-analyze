...
def render_count_analysis(f_data):
    ...
    if count_list:
        fig = px.bar(pd.DataFrame(count_list), x='カウント', y='割合(%)', color='球種', color_discrete_map=PITCH_COLORS, category_orders={'球種': get_safe_order(target_df)})
...
def render_risk_management(f_data):
    ...
    def classify_result(row):
        ...
        if str(row.get('PitchCall', '')).upper() == 'STRIKESWINGING': return '空振り'
        return 'その他'
...
def render_stats_tab(f_data, mode="full"):
    ...
    summary = f_data.groupby('TaggedPitchType').agg({'RelSpeed': ['count', 'mean', 'max'], 'is_strike': 'mean', 'is_whiff': 'sum', 'is_swing': 'sum'})
    summary.columns = ['count', 'mean', 'max', 'strike_rate', 'whiff_sum', 'swing_sum']
    summary = summary.reindex(get_safe_order(f_data)).dropna(subset=['count'])

    disp = pd.DataFrame(index=summary.index)
    disp['投球数'] = summary['count'].astype(int)
    disp['割合'] = (summary['count'] / summary['count'].sum() * 100).apply(lambda x: f"{x:.1f}%")
    disp['平均球速'] = summary['mean'].apply(lambda x: f"{x:.1f}")
    disp['最速'] = summary['max'].apply(lambda x: f"{x:.1f}")
    if mode == "full":
        disp['ストライク率'] = (summary['strike_rate'] * 100).apply(lambda x: f"{x:.1f}%")
        disp['Whiff%'] = (summary['whiff_sum'] / summary['swing_sum'].replace(0,1) * 100).apply(lambda x: f"{x:.1f}%")

    col_l, col_r = st.columns([2.6, 1])
    with col_l: st.table(disp)
    with col_r:
        fig, ax = plt.subplots(figsize=(2.8, 2.8))
        ax.pie(summary['count'], labels=summary.index, autopct='%1.1f%%', colors=[PITCH_COLORS.get(s, "#9EDAE5") for s in summary.index], startangle=90, counterclock=False)
        st.pyplot(fig)
...
if df is not None:
    ...
    for i, cat in enumerate(["SBP", "紅白戦", "オープン戦", "実戦/PBP", "pitching"]):
        with tabs[i]:
            ...
            c1, c2 = st.columns(2)
            p = c1.selectbox("投手", ["すべて"] + sorted(sub['Pitcher'].unique().tolist()), key=f"p_{i}")
            p_sub = sub if p == "すべて" else sub[sub['Pitcher'] == p]
            d = c2.selectbox("日付", ["すべて"] + sorted([str(d) for d in p_sub['Date'].unique()], reverse=True), key=f"d_{i}")
            final = p_sub if d == "すべて" else p_sub[p_sub['Date'].astype(str) == d]
            render_stats_tab(final, mode=("pitching" if cat=="pitching" else "full"))
